...
class OursSupervisedModel(training.base_trainer.BaseTrainer):
    def __init__(self, settings, train=True):
        ...
        self.right_count = 0
        self.wrong_count = 0
        self.categories = ['flat', 'background','object','vegetation','human','vehicle']
    ...
    def init_fn(self):
        ...
        self.metrics_semseg_b_2 = MetricsSemseg(self.settings.semseg_num_classes, self.settings.semseg_ignore_label,
                                              self.settings.semseg_class_names)
    ...
    def task_train_step(self, batch, init_func=False):
        ...
        data_B = batch[2].to(data_A.device)
        
        # return event_tensor, img_tensor, label_tensor, label_original_tensor
        # return event_tensor, sparse_label_tensor, event_tensor, label_tensor
        ...
        if self.epoch_count >= 2:
            if valid_A:
                ...
                loss_all += 0.1 * loss_proto_A/len(proto_opts)
            ...
            if valid_B:
                ...
                loss_all += 0.1 * loss_proto_B/len(proto_opts)
        
        
        if self.epoch_count >= 2:
            if valid_A:
                if 2 in proto_opts:
                    loss_proto_A += nce_loss_A2
                if 4 in proto_opts:
                    loss_proto_A += nce_loss_A4
                if 8 in proto_opts:
                    loss_proto_A += nce_loss_A8
                loss_all += 0.1 * loss_proto_A/len(proto_opts)

            if valid_B:
                if 2 in proto_opts:
                    loss_proto_B += nce_loss_B2 
                if 4 in proto_opts:
                    loss_proto_B += nce_loss_B4 
                if 8 in proto_opts:
                    loss_proto_B += nce_loss_B8 
        ...
        if self.epoch_count >= 2:
            loss_all += 0.1 * loss_distill/(len(distill_opts)*2)
        ...
        # if True:
            ...
            self.visTaskStep(data_A, pred_A, dense_labels_A, _pred_A)
    ...
    def visTaskStep(self, data, pred, labels,  cam):
        # pred = pred[1]
        ...
        # else:
        pred = F.interpolate(
            pred, size=(data.size()[2:]), mode="bilinear", align_corners=True
        )
    ...
    def validationEpoch(self, data_loader, sensor_name):
        ...
        if sensor_name == 'sensor_a':
            ...
            metrics_semseg_b_2 = self.metrics_semseg_b_2.get_metrics_summary()
            metric_semseg_b_2_mean_iou = metrics_semseg_b_2['mean_iou']
            cumulative_losses['semseg_sensor_b_reverse_mean_iou'] = metric_semseg_b_2_mean_iou
            metric_semseg_b_2_acc = metrics_semseg_b_2['acc']
            cumulative_losses['semseg_sensor_b_reverse_acc'] = metric_semseg_b_2_acc
    ...
    def val_step(self, input_batch, sensor, i_batch, vis_reconstr_idx):
        ...
        pred_lbl = self.valTaskStep(data, labels, losses, sensor, reverse_data)
    ...
    def valTaskStep(self, data, labels, losses, sensor, reverse_data):
        """Computes the task loss and updates metrics"""
        # task_backend = self.models_dict["back_end"]
        enc_net = self.models_dict["enc1"]
        task_backend = self.models_dict["dec1"]
        enc_net_2 = self.models_dict["enc2"]
        task_backend_2 = self.models_dict["dec2"]
        ...
        pred = F.interpolate(
            pred, size=(labels.size()[1:]), mode="bilinear", align_corners=True
        )
        ...
        states_real = None
        for i in range(self.settings.nr_events_data_b):
            reverse_event_tensor = reverse_data[:, i * self.settings.input_channels_b:(i + 1) * self.settings.input_channels_b, :, :]
            states_real, latent_real = enc_net_2(event_tensor, states_real)
        pred_reverse = task_backend_2(latent_real)[1]
        
        # pred = F.interpolate(
        #     pred, size=(self.settings.img_size_b), mode="bilinear", align_corners=True
        # )

        pred_reverse = F.interpolate(
            pred_reverse, size=(labels.size()[1:]), mode="bilinear", align_corners=True
        )
        
        pred_lbl_reverse = pred_reverse.argmax(dim=1)
        ...
        if sensor == 'sensor_a':
            ...
            self.metrics_semseg_b_2.update_batch(pred_lbl_reverse, labels)
