...
class BaseTrainer(object):
    ...
    def __init__(self, settings):
        ...
        self.buildModels()
        self.init_loss()
        self.models_dict = {k: nn.DataParallel(v).cuda() for k, v in self.models_dict.items()}
        # self.models_dict = {k: v.to(self.device) for k, v in self.models_dict.items()}
        
       
        ### just for patchnce loss
        # self.trainInit()
    ...
    def getDataloader(self, dataset_name):
        ...
        if dataset_name == 'DSEC_events':
            ...
            return DSECEvents
        elif dataset_name == 'DSEC_events_hh':
            from datasets.DSEC_events_loader import DSECEventsHH
            return DSECEventsHH
        elif dataset_name == 'DSEC_events_weak':
            from datasets.DSEC_events_loader import DSECEventsWeak
            return DSECEventsWeak
        elif dataset_name == 'DSEC_events_weak_aug':
            from datasets.DSEC_events_loader import DSECEventsWeakAug
            return DSECEventsWeakAug
        elif dataset_name == 'DSEC_events_weak_aug_night':
            from datasets.DSEC_events_loader import DSECEventsWeakAugNight
            return DSECEventsWeakAugNight
        elif dataset_name == 'DSEC_events_weak_aug_voxel':
            from datasets.DSEC_events_loader import DSECEventsWeakAugVoxel
            return DSECEventsWeakAugVoxel
    ...
    def createDSECDatasetAug(self, dataset_name, dsec_dir, batch_size, nr_events_data, delta_t_per_data, nr_events_window,
                          augmentation, event_representation, nr_bins_per_data, require_paired_data_train,
                          require_paired_data_val,
                          separate_pol, normalize_event, semseg_num_classes, fixed_duration):
        """
        ...
        train_dataset = dataset_builder(dsec_dir=dsec_dir,
                                        ...
                                        semseg_num_classes=semseg_num_classes,
                                        fixed_duration=fixed_duration,
                                        short_mul=self.settings.short_mul,
                                        random_crop=self.settings.random_crop,
    ...
    def createDSECDatasetAugNight(self, dataset_name, dsec_dir, dsec_dense_dir, batch_size, nr_events_data, delta_t_per_data, nr_events_window,
                          augmentation, event_representation, nr_bins_per_data, require_paired_data_train,
                          require_paired_data_val,
                          separate_pol, normalize_event, semseg_num_classes, fixed_duration):
        """
    ...
    def createDSECDatasetAugVoxel(self, dataset_name, dsec_dir, batch_size, nr_events_data, delta_t_per_data, nr_events_window,
                          augmentation, event_representation, nr_bins_per_data, require_paired_data_train,
                          require_paired_data_val,
                          separate_pol, normalize_event, semseg_num_classes, fixed_duration, ten_point):
        """
        Creates the validation and the training data based on the provided paths and parameters.
        """
        dataset_builder = self.getDataloader(dataset_name)
    ...
    def createDDD17EventsDataset(self, dataset_name, root, split_train, batch_size, nr_events_data, delta_t_per_data,
                                 ...
                                 normalize_event, fixed_duration, ten_point):
        """
        Creates the validation and the training data based on the provided paths and parameters.
        """
        dataset_builder = self.getDataloader(dataset_name)
    ...
    def combineDataloaders(self):
        ...
        self.train_loader = self.train_loader_sensor_b
    ...
    def createDataLoaders(self):
        ...
        elif self.settings.dataset_name_b == 'DSEC_events_weak_aug':
            out = self.createDSECDatasetAug(self.settings.dataset_name_b,
                                         self.settings.dataset_path_b,
                                         self.settings.batch_size_b,
                                         self.settings.nr_events_data_b,
                                         self.settings.delta_t_per_data_b,
                                         ...
                                         self.settings.semseg_num_classes,
                                         self.settings.fixed_duration_b)
        elif self.settings.dataset_name_b == 'DSEC_events_weak_aug_night':
            out = self.createDSECDatasetAugNight(self.settings.dataset_name_b,
                                         self.settings.dataset_path_b,
                                         self.settings.dataset_path_b_dense,
                                         self.settings.batch_size_b,
                                         self.settings.nr_events_data_b,
                                         self.settings.delta_t_per_data_b,
                                         self.settings.nr_events_window_b,
                                         ...
                                         self.settings.semseg_num_classes,
                                         self.settings.fixed_duration_b)
        elif self.settings.dataset_name_b == 'DSEC_events_weak_aug_voxel':
            out = self.createDSECDatasetAugVoxel(self.settings.dataset_name_b,
                                         self.settings.dataset_path_b,
                                         self.settings.batch_size_b,
                                         self.settings.nr_events_data_b,
                                         self.settings.delta_t_per_data_b,
    ...
    def train(self):
        ...
        for _ in tqdm(range(self.epoch_count, self.settings.num_epochs), total=self.settings.num_epochs,
            ...
            if self.settings.resume_training:
                if self.epoch_count==0 or ((self.epoch_count-1) % val_epoch_step)==0:
                   self.validationEpochs()
            else:
                if (self.epoch_count % val_epoch_step) == 0:
                    self.validationEpochs()
            ...
            if self.settings.save_checkpoint:
                ...
                if self.best_save:
                    if self.settings.model_name == 'generated_events':
                        self.saver.save_checkpoint_best(self.task_models_dict,
                                                   self.optimizers_dict, self.epoch_count, self.step_count,
                                                   self.settings.batch_size_a,
                                                   self.settings.batch_size_b)
                    else:
                        self.saver.save_checkpoint_best(self.models_dict,
                                                   self.optimizers_dict, self.epoch_count, self.step_count,
                                                   self.settings.batch_size_a,
                                                   self.settings.batch_size_b)
    ...
    def trainInit(self):
        ...
        for i_batch, sample_batched in enumerate(self.train_loader):
            self.train_init_step(sample_batched)
            break
            self.train_summaries(out[0])
    ...
    def cam_summaries(self, tag, img, step=None):
        self.summary_writer.add_image(tag, img, step)
        self.summary_writer.flush()
