...
from DSEC.dataset.provider import DatasetProvider, DatasetProviderWeakAug, DatasetProviderWeakAugVoxel, DatasetProviderWeakAugNight
...
def DSECEventsWeakAug(dsec_dir, nr_events_data=1, delta_t_per_data=50, nr_events_window=-1,
               augmentation=False, mode='train', task='segmentation', event_representation='voxel_grid',
               nr_bins_per_data=5, require_paired_data=False, separate_pol=False, normalize_event=False,
               semseg_num_classes=11, fixed_duration=False, resize=False, short_mul=0.8, random_crop=False,
               crop_size=384):
    """
    Creates an iterator over the EventScape dataset.
    ...
    dataset_provider = DatasetProviderWeakAug(dsec_dir, mode, event_representation=event_representation,
                                       nr_events_data=nr_events_data, delta_t_per_data=delta_t_per_data,
                                       nr_events_window=nr_events_window, nr_bins_per_data=nr_bins_per_data,
                                       require_paired_data=require_paired_data, normalize_event=normalize_event,
                                       separate_pol=separate_pol, semseg_num_classes=semseg_num_classes,
                                       augmentation=augmentation, fixed_duration=fixed_duration, resize=resize,
                                       short_mul=short_mul, random_crop=random_crop, crop_size=crop_size)
    if mode == 'train':
        train_dataset = dataset_provider.get_train_dataset()
        return train_dataset
    else:
...
def DSECEventsWeakAugNight(dsec_dir, dsec_dense_dir, nr_events_data=1, delta_t_per_data=50, nr_events_window=-1,
               augmentation=False, mode='train', task='segmentation', event_representation='voxel_grid',
               nr_bins_per_data=5, require_paired_data=False, separate_pol=False, normalize_event=False,
               semseg_num_classes=11, fixed_duration=False, resize=False, short_mul=0.8, random_crop=False,
               crop_size=384):
    ...
    dataset_provider = DatasetProviderWeakAugNight(dsec_dir, dsec_dense_dir, mode, event_representation=event_representation,
                                       nr_events_data=nr_events_data, delta_t_per_data=delta_t_per_data,
                                       nr_events_window=nr_events_window, nr_bins_per_data=nr_bins_per_data,
                                       require_paired_data=require_paired_data, normalize_event=normalize_event,
                                       separate_pol=separate_pol, semseg_num_classes=semseg_num_classes,
...
def DSECEventsWeakAugVoxel(dsec_dir, nr_events_data=1, delta_t_per_data=50, nr_events_window=-1,
               augmentation=False, mode='train', task='segmentation', event_representation='voxel_grid',
               nr_bins_per_data=5, require_paired_data=False, separate_pol=False, normalize_event=False,
               semseg_num_classes=11, fixed_duration=False, resize=False, short_mul=0.8, random_crop=False,
               crop_size=384, ten_point=False):
    """
    Creates an iterator over the EventScape dataset.
    ...
    dataset_provider = DatasetProviderWeakAugVoxel(dsec_dir, mode, event_representation=event_representation,
                                       nr_events_data=nr_events_data, delta_t_per_data=delta_t_per_data,
                                       nr_events_window=nr_events_window, nr_bins_per_data=nr_bins_per_data,
                                       require_paired_data=require_paired_data, normalize_event=normalize_event,
                                       separate_pol=separate_pol, semseg_num_classes=semseg_num_classes,
