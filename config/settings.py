...
class Settings:
    def __init__(self, settings_yaml, generate_log=True):
        ...
        with open(settings_yaml, 'r') as stream:
            ...
            self.gpu_device = torch.device("cpu") if gpu_device == "cpu" else torch.device("cuda")
            ...
            if self.dataset_name_b in ['EventScape_recurrent_events', 'DSEC_events', 'DSEC_events_hh', 'DSEC_events_weak', 'DSEC_events_weak_aug', 
                                       'DSEC_events_weak_aug_night', 'DSEC_events_weak_aug_voxel', 'DDD17_events', 'E2VIDDriving_events']:
                if self.dataset_name_b == 'DSEC_events':
                ...
                elif self.dataset_name_b == 'DSEC_events_hh':
                    dataset_specs = dataset['DSEC_events_hh']
                    self.delta_t_per_data_b = dataset_specs['delta_t_per_data']
                    self.semseg_label_train_b = False
                    self.semseg_label_val_b = True
                elif self.dataset_name_b == 'DSEC_events_weak':
                    dataset_specs = dataset['DSEC_events_weak']
                    self.delta_t_per_data_b = dataset_specs['delta_t_per_data']
                    self.semseg_label_train_b = False
                    self.semseg_label_val_b = True
                elif self.dataset_name_b == 'DSEC_events_weak_aug':
                    dataset_specs = dataset['DSEC_events_weak_aug']
                    self.delta_t_per_data_b = dataset_specs['delta_t_per_data']
                    self.semseg_label_train_b = False
                    self.semseg_label_val_b = True
                    self.short_mul = dataset_specs['short_mul']
                    self.random_crop = dataset_specs['random_crop']
                    self.crop_size = dataset_specs['crop_size']
                elif self.dataset_name_b == 'DSEC_events_weak_aug_night':
                    dataset_specs = dataset['DSEC_events_weak_aug_night']
                    self.delta_t_per_data_b = dataset_specs['delta_t_per_data']
                    self.semseg_label_train_b = False
                    self.semseg_label_val_b = True
                    self.short_mul = dataset_specs['short_mul']
                    self.random_crop = dataset_specs['random_crop']
                    self.crop_size = dataset_specs['crop_size']
                    self.dataset_path_b_dense = dataset_specs['dataset_dense_path']
                elif self.dataset_name_b == 'DSEC_events_weak_aug_voxel':
                    dataset_specs = dataset['DSEC_events_weak_aug_voxel']
                    self.delta_t_per_data_b = dataset_specs['delta_t_per_data']
                    self.semseg_label_train_b = False
                    self.semseg_label_val_b = True
                    self.short_mul = dataset_specs['short_mul']
                ...
                self.ten_point = dataset_specs['ten_point']
            ...
            if self.semseg_num_classes == 6:
                ...
                self.semseg_ignore_label = 255
                self.semseg_class_names = ['road', 'sidewalk', 'building', 'wall', 'fence', 'pole', 'traffic_light', 'trafic_sign', 'vegetation',
                                           'terrain', 'sky', 'person', 'rider', 'car', 'truck', 'bus', 'train', 'motorcycle', 'bicycle']
                self.semseg_color_map = np.zeros((self.semseg_num_classes, 3), dtype=np.uint8)
                self.semseg_color_map[0] = [128, 64, 128]
                self.semseg_color_map[1] = [244, 35, 232]
                self.semseg_color_map[2] = [70, 70, 70]
                self.semseg_color_map[3] = [102, 102, 156]
                self.semseg_color_map[4] = [190, 153, 153]
                self.semseg_color_map[5] = [153, 153, 153]
                self.semseg_color_map[6] = [250, 170, 30]
                self.semseg_color_map[7] = [220, 220, 0]
                self.semseg_color_map[8] = [107, 142, 35]
                self.semseg_color_map[9] = [152, 251, 152]
                self.semseg_color_map[10] = [70, 130, 180]
                self.semseg_color_map[11] = [220, 20, 60]
                self.semseg_color_map[12] = [255, 0, 0]
                self.semseg_color_map[13] = [0, 0, 142]
                self.semseg_color_map[14] = [0, 0, 70]
                self.semseg_color_map[15] = [0, 60, 100]
                self.semseg_color_map[16] = [0, 80, 100]
                self.semseg_color_map[17] = [0, 0, 230]
                self.semseg_color_map[18] = [119, 11, 32]
            elif self.semseg_num_classes == 18:
                self.semseg_ignore_label = 255
                self.semseg_class_names = ['road', 'sidewalk', 'building', 'wall', 'fence', 'pole', 'traffic_light', 'trafic_sign', 'vegetation',
                                           'terrain', 'sky', 'person', 'rider', 'car', 'bus', 'train', 'motorcycle', 'bicycle']
                self.semseg_color_map = np.zeros((self.semseg_num_classes, 3), dtype=np.uint8)
                self.semseg_color_map[0] = [128, 64, 128]
                self.semseg_color_map[1] = [244, 35, 232]
                self.semseg_color_map[2] = [70, 70, 70]
                ...
                self.semseg_color_map[12] = [255, 0, 0]
                self.semseg_color_map[13] = [0, 0, 142]
                self.semseg_color_map[14] = [0, 60, 100]
                self.semseg_color_map[15] = [0, 80, 100]
                self.semseg_color_map[16] = [0, 0, 230]
                self.semseg_color_map[17] = [119, 11, 32]
            ...
            # --- logs ---
            if generate_log:
                if 'exp_name' in settings.keys():
                    log_dir = os.path.join(log_dir, settings['exp_name'])
                else:
            ...
            self.lr_cls = float(optimization['lr_cls']) #
            ...
            if 'sim_loss_weight' in optimization.keys():
                self.sim_loss_weight = optimization['sim_loss_weight']
                
            # --- loss ---
            loss = settings['loss']
            if 'dual_thres' in loss.keys():
                self.dual_thres = loss['dual_thres']
            if 'sim_weight' in loss.keys():
                self.sim_weight = loss['sim_weight']
