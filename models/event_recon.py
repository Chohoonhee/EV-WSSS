...
resize = transforms.Resize(256, interpolation=Image.BILINEAR)#, max_size=None, antialias='warn')
...
class QuantizationLayer(nn.Module):
    ...
    def forward(self, events, aug=False, aug_temp=False, for_cont=False, vox_dim = None):
        # if 82340 in b_idx:
        #     pdb.set_trace()
        ...
        num_voxels = int(2 * np.prod(vox_dim) * B)
        ...
        # vox = self.conv(vox)
        if H >= 440:
            vox, pad = self.crop_and_resize_to_resolution(vox, (H, W))
...
class Event_Reconst(nn.Module):
    def __init__(self,
                #  voxel_dimension=(9,180,240),  # dimension of voxel will be C x 2 x H x W
                 voxel_dimension=(9,256,256),  # dimension of voxel will be C x 2 x H x W
    ...
    def forward(self, x, aug=False, aug_temp=False, for_cont=False, voxel_dim = None):
        ...
        if aug:
            ...
            vox = self.quantization_layer.forward(x, aug=aug, vox_dim = voxel_dim)
        return vox
