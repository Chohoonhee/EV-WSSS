...
from datasets.extract_data_tools.example_loader_ddd17 import load_files_in_directory, extract_events_from_memmap, extract_events_from_memmap_reverse
...
class DDD17Events(Dataset):
    def __init__(self, root, split="train", event_representation='voxel_grid',
                 ...
                 nr_events_per_data=32000, resize=True, random_crop=False, ten_point=False):
        data_dirs = sorted(glob.glob(join(root, "dir*")))
        assert len(data_dirs) > 0
        assert split in ["train", "valid", "test"]
        self.ten_point = ten_point
        self.split = split
    ...
    def apply_augmentation(self, transform_a, events, reverse_events, label, sparse_label):
        label = shiftUpId(label)
        sparse_label = shiftUpId(sparse_label)
        A_data = transform_a(image=events[0, :, :].numpy(), image0=reverse_events[0, :, :].numpy(), mask=label, mask0=sparse_label)
        
        label = A_data['mask']
        label = shiftDownId(label)
        sparse_label = A_data['mask0']
        sparse_label = shiftDownId(sparse_label)
        
        
        if self.random_crop and self.split == 'train':
            events_tensor = torch.zeros((events.shape[0], self.shape_crop[0], self.shape_crop[1]))
            reverse_events_tensor = torch.zeros((reverse_events.shape[0], self.shape_crop[0], self.shape_crop[1]))
        else:
            events_tensor = events
            reverse_events_tensor = reverse_events
        ...
        for k in range(events.shape[0]):
            reverse_events_tensor[k, :, :] = torch.from_numpy(
                A.ReplayCompose.replay(A_data['replay'], image=reverse_events[k, :, :].numpy())['image'])
            

        return events_tensor, reverse_events_tensor, label, sparse_label
    ...
    def __getitem__(self, idx):
        ...
        if self.ten_point:
            sparse_segmentation_mask_file = segmentation_mask_file.replace('segmentation_masks', 'weak_10point_per_class')
        else:
            sparse_segmentation_mask_file = segmentation_mask_file.replace('segmentation_masks', 'weak_1point_per_class')


        sparse_mask = cv2.imread(sparse_segmentation_mask_file, 0)
        sparse_label_original = np.array(sparse_mask)
        
        if self.resize:
            segmentation_mask = cv2.resize(segmentation_mask, (self.shape_resize[1], self.shape_resize[0] - 60),
                                           interpolation=cv2.INTER_NEAREST)
            sparse_mask = cv2.resize(sparse_mask, (self.shape_resize[1], self.shape_resize[0] - 60),
                                           interpolation=cv2.INTER_NEAREST)
            
            
        label = np.array(segmentation_mask)
        sparse_label = np.array(sparse_mask)
        ...
        reverse_events = extract_events_from_memmap_reverse(t_events, xyp_events, img_idx, img_timestamp_event_idx, self.fixed_duration,
                                            self.nr_events)
        # reverse_events = extract_events_from_memmap(t_events, xyp_events, img_idx, img_timestamp_event_idx, self.fixed_duration,
        #                                     self.nr_events)
        ...
        #### reverse events
        t_ns = reverse_events[:, 2]
        delta_t_ns = int((t_ns[-1] - t_ns[0]) / self.nr_events_data)
        nr_events_loaded = reverse_events.shape[0]
        nr_events_temp = nr_events_loaded // self.nr_events_data


        id_end = 0
        reverse_event_tensor = None
        for i in range(self.nr_events_data):
            id_start = id_end
            if self.fixed_duration:
                id_end = np.searchsorted(t_ns, t_ns[0] + (i + 1) * delta_t_ns)
            ...
            if id_end > nr_events_loaded:
                id_end = nr_events_loaded
            event_representation = data_util.generate_input_representation(reverse_events[id_start:id_end],
                                                                           self.event_representation,
                                                                           self.shape,
                                                                           nr_temporal_bins=self.nr_temporal_bins,
                                                                           separate_pol=self.separate_pol)
            ...
            if reverse_event_tensor is None:
                reverse_event_tensor = event_representation
            else:
                reverse_event_tensor = torch.cat([reverse_event_tensor, event_representation], dim=0)

        reverse_event_tensor = reverse_event_tensor[:, :-60, :]  # remove 60 bottom rows


        if self.random_crop and self.split == 'train':
            event_tensor = event_tensor[:, -self.shape_crop[0]:, :]
            label = label[-self.shape_crop[0]:, :]
            sparse_label = sparse_label[-self.shape_crop[0]:, :]
            reverse_event_tensor = reverse_event_tensor[:, -self.shape_crop[0]:, :]
            
            
            if self.augmentation:
                event_tensor, reverse_event_tensor, label, sparse_label = self.apply_augmentation(self.transform_a_random_crop, event_tensor, reverse_event_tensor, label, sparse_label)

        else:
            if self.augmentation:
                event_tensor, reverse_event_tensor, label, sparse_label = self.apply_augmentation(self.transform_a, event_tensor, reverse_event_tensor, label, sparse_label)


        label_tensor = torch.from_numpy(label).long()
        sparse_label_tensor = torch.from_numpy(sparse_label).long()
        ...
        if self.split == 'valid' and self.require_paired_data:
            ...
            return event_tensor, img_tensor, label_tensor, label_original_tensor, reverse_event_tensor
        return event_tensor, sparse_label_tensor, reverse_event_tensor, label_tensor
