...
def extract_events_from_memmap_reverse(t_events, xyp_events, img_idx, img_timestamp_event_idx, fixed_duration=False,
                               nr_events=32000):
    # timestep, event_idx = img_timestamp_event_idx[img_idx]
    # _, event_idx_before = img_timestamp_event_idx[img_idx - 1]
    if fixed_duration:
        ...
        timestep, event_idx, _ = img_timestamp_event_idx[img_idx]
        event_idx_after = min([event_idx + nr_events, len(t_events)])
    
    t = t_events[event_idx-1:event_idx_after]
    xyp_events = xyp_events[event_idx-1:event_idx_after]
    ...
    events_between_imgs = np.concatenate([
        np.array(t, dtype="int64"),
        np.array(np.stack([x,y,p], axis=1), dtype="int64")
    ], -1)
    events_between_imgs = events_between_imgs[:, [1, 2, 0, 3]]  # events have format xytp, and p is in [0,1]
