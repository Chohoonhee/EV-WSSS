...
class CheckpointSaver(object):
    ...
    # save checkpoint
    def save_checkpoint_best(self, models, optimizers, epoch, step_count, batch_size_a, batch_size_b):
        timestamp = datetime.datetime.now()
        checkpoint_filename = os.path.abspath(os.path.join(self.save_dir, 'best_epoch.pt'))
        checkpoint = {}
        for model in models:
            checkpoint[model] = models[model].state_dict()
        for optimizer in optimizers:
