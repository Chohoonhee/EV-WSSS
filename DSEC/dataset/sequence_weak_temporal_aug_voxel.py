...
class Sequence(Dataset):
    ...
    def __init__(self, seq_path: Path, mode: str='train', event_representation: str = 'voxel_grid',
                 ...
                 resize=False, crop=False, short_mul=0.8, crop_size=[384,384], ten_point=False):
        ...
        if mode == 'train':
            if self.semseg_num_classes == 11:
                ...
                # label_dir = seq_path / '11classes_weak'
                    label_dir = seq_path / '11classes_weak_10point_per_class'
                else:
        ...
        if self.mode == 'train':
            voxel_dir = seq_path 
            voxel_left_dir = voxel_dir / 'left' / 'voxel'
        ...
        if self.mode == 'train':
            # del self.voxel_left_pathstrings[:(self.remove_time_window // 100 + 1) * 2]
            # assert len(self.img_left_pathstrings) == len(self.voxel_left_pathstrings)
            assert (len(self.label_pathstrings)+1)//2 == len(self.voxel_left_pathstrings)
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            if self.ten_point:
                dense_label_path = Path(str(label_path).replace('_weak_10point_per_class', ''))
            else:
        ...
        for location in self.locations:
            if self.mode != 'train':
                ...
                voxel_path = self.voxel_left_pathstrings[index]
                event_tensor = np.load(voxel_path)
                
                reverse_path = voxel_path.replace('voxel', 'reverse_voxel_5')
                short_event_tensor = np.load(reverse_path)
                
                event_tensor = torch.tensor(event_tensor)
                short_event_tensor = torch.tensor(short_event_tensor)
            
            # pdb.set_trace()
            # print(voxel_path, label_path)
        ...
        if self.require_paired_data:
            ...
            if self.mode == 'train':
                return output['representation']['left'], output['img_left'], label_tensor, short_event_tensor, dense_label_tensor
            else:
        ...
        if self.mode == 'train':
            return output['representation']['left'], label_tensor, short_event_tensor, dense_label_tensor
        else:
