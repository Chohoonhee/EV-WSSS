...
class EventSlicer:
    ...
    def get_events_fixed_num_reverse(self, t_end_us: int, nr_events: int = 100000) -> Dict[str, np.ndarray]:
        """Get events (p, x, y, t) with fixed number of events
        Parameters
        ----------
        t_end_us: end time in microseconds
        ...
        if t_start_us_idx >= len(self.events['x']):
            t_start_us_idx = len(self.events['x']) - 1

        for dset_str in self.events.keys():
            events[dset_str] = np.asarray(self.events[dset_str][t_end_us_idx:t_start_us_idx])
