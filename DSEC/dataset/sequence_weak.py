...
class Sequence(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for location in self.locations:
            ...
            if self.crop:
                crop_size = 256
                x0 = np.random.randint(0, self.width - crop_size)
                y0 = np.random.randint(0, self.height - 40 - crop_size)
                event_tensor = event_tensor[:, y0:y0+crop_size, x0:x0+crop_size]
                label_tensor = label_tensor[y0:y0+crop_size, x0:x0+crop_size]     
                if self.mode == 'train':
                    dense_label_tensor = dense_label_tensor[y0:y0+crop_size, x0:x0+crop_size]
        ...
        if self.require_paired_data:
            ...
            output['img_left'] = img
            if self.mode == 'train':
                return output['representation']['left'], output['img_left'], label_tensor, dense_label_tensor
            else:
                return output['representation']['left'], output['img_left'], label_tensor
        if self.mode == 'train':
            return output['representation']['left'], label_tensor, dense_label_tensor
        else:
